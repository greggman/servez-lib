...
// Needed beacuse Kodi's bugs
const escapeQuotes = (function() {
  const aposRE = /'/g;
  const quoteRE = /"/g;
  return function escapeQuotes(s) {
    return s.replace(aposRE, '%27').replace(quoteRE, '%22');
  };
}());
...
function createHtmlFileList(files, dir) {
  ...
  return `<table id="files">${
    files.map(file => {
      ...
      const url = `${escapeHtml(escapeQuotes(pathname))}${endSlash}`;
